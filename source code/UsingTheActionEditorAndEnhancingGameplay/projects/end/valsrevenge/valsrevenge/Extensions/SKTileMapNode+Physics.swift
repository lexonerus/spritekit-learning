      ...
      for row in 0..<numberOfRows {
        if let td = tileDefinition(atColumn: col, row: row) {
          ...
          if let bodySizeValue = td.userData?.value(forKey: "bodySize")
            ...
            tileNode.physicsBody?.categoryBitMask =
              PhysicsBody.wall.categoryBitMask
            tileNode.physicsBody?.collisionBitMask =
              PhysicsBody.wall.collisionBitMask
            tileNode.physicsBody?.contactTestBitMask =
              PhysicsBody.wall.contactTestBitMask
